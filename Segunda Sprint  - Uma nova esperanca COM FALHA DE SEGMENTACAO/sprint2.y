...
%token TK_BREAK TK_EMPILHA TK_DESEMPILHA
...
E 			: E TK_SOMA E
			...
			| TK_DO '{' COMANDOS '}' TK_WHILE '(' E ')'
			{
				if($7.tipo == "boolean")
				{
					string *label = criaLabel("loop");

					$$.traducao   = "\n\t" + label[0] + ":\n" 
										   + $3.traducao + $7.traducao + "\n\tif(!" + $7.label + ") goto " + label[1] + ";\n\t"
										   + "goto " + label[0] + ";\n" + "\n\t" + label[1] + ":\n"; 
				}
			}
			...
			| TK_BREAK
			{
				...
				cout <<"labels labels bebels "<< qtdLabels << endl;
/*
				if(qtdLabels == 0)
				{
					stringstream qtd;
